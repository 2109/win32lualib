...
public:
	...
	void call_oneIntParam_calledOnceWithCorrectParam()
	{
		doScript("one", 1);
		MockHelper helper;
		EXPECT_CALL(helper.mock, one(::testing::Eq(helper.p1))).Times(1);
	...
	void call_twoIntParam_calledOnceWithCorrectParams()
	{
		doScript("two", 2);
		MockHelper helper;
		EXPECT_CALL(helper.mock, two(::testing::Eq(helper.p1), ::testing::Eq(helper.p2))).Times(1);
	...
	void call_threeIntParam_calledOnceWithCorrectParams()
	{
		...
		EXPECT_CALL(helper.mock
			...
			.Times(1);
		m_lua->call("func", helper.mockBase, helper.p1, helper.p2, helper.p3);
	}
