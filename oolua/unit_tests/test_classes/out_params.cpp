...
public:
	...
	void inOutTrait_luaPassesIntFunctionWantsRefToInt_calledWithExpectedValue()
	{
		...
		EXPECT_CALL(mock, int_ref(::testing::Eq(expected))).Times(1);
		m_lua->call("func", static_cast<OutParamsTest*>(&mock));
	}
	...
	void inOutTrait_luaPassesIntFunctionWantsRefToInt_returnIsInput()
	{
		...
		m_lua->call(1, static_cast<OutParamsTest*>(&stub), expected);
	...
	void outTrait_luaPassesNoParamFunctionWantsIntPtr_calledOnce()
	{
		...
		MockOutParamsTest mock;
		EXPECT_CALL(mock, int_ptr(::testing::_)).Times(1);
		m_lua->call("func", static_cast<OutParamsTest*>(&mock));
	}
	...
	void outTrait_luaPassesNoParamFunctionWantsRefToInt_valueAtTopOfStackIsExpectedValue()
	{
		...
		m_lua->call(1, static_cast<OutParamsTest*>(&stub));
		assert_top_of_stack_is_expected_value(expected);
	}
	/**[TestOutTraitExampleFunction]*/
	...
	void noTraits_luaPassesIntFunctionWantsIntPtr_callsFunctionWithCorrectPointeeValue()
	{
		...
		MockOutParamsTest mock;
		EXPECT_CALL(mock, int_ptr(::testing::Pointee(input_param))).Times(1);
		m_lua->call("func", static_cast<OutParamsTest*>(&mock));
	}
	...
	void noTraits_luaPassesIntFunctionWantsIntPtr_afterCallTheStackIsTheSameAsBeforeCall()
	{
		...
		run_chunk_function_push_int("int_ptr", input_param, true);
	...
	void OutTrait_luaPassesNoParamFunctionWantsRefToUserData_topOfStackIsOwnedByLua()
	{
		...
		m_lua->call("foo", static_cast<OutParamsUserData*>(&stub));
